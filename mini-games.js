...
(function () {
    ...
    function showHub() {
        ...
        QuizGame.reset();
        PasalGame.reset();
        // Flush skor flappy yang belum ter-submit (kalau user kembali ke hub
        // di tengah match) — supaya skor TIDAK HILANG.
        try {
            if (window.FlappyGame && typeof window.FlappyGame.flushPendingScore === 'function') {
                window.FlappyGame.flushPendingScore('back-to-hub');
            }
        } catch (e) { /* ignore */ }
    ...
    document.addEventListener('click', function (e) {
        ...
        if (launchBtn) {
            e.preventDefault();
            const key = launchBtn.getAttribute('data-launch');
            // GUARD: User HARUS isi profil (nama + sekolah) dulu sebelum main.
            // Tanpa nama → submit skor di-skip (playerName='Pemain') → skor
            // tidak masuk leaderboard. Ini bug yang bikin user kebingungan
            // ("kok skor 2000 di leaderboard masih 0?"). Paksa modal profil
            // muncul kalau belum diisi.
            try {
                if (typeof HubProfile !== 'undefined' && HubProfile && typeof HubProfile.hasProfile === 'function' && !HubProfile.hasProfile()) {
                    if (typeof HubModal !== 'undefined' && HubModal && typeof HubModal.open === 'function') {
                        HubModal.open('name');
                        return;
                    }
                }
            } catch (err) { /* ignore — lanjut launch */ }
            launchGame(key);
            return;
        }
    ...
    const HubProfile = (function () {
        ...
        // Cache-aware leaderboard fetch.
        //   opts.force === true → bypass cache localStorage 30s
        //                         (dipakai saat user klik tombol Refresh).
        //   default              → pakai cache; cegah burst request
        //                         Supabase saat 300+ siswa buka hub.
        function fetchLeaderboard(opts) {
            ...
            const p = window.EtingDB.getLeaderboard(10, opts || {})
        ...
        function bind() {
            ...
            if (els.btnRefresh) {
                els.btnRefresh.addEventListener('click', function (e) {
                    e.preventDefault();
                    // User EKSPLISIT mau data fresh — bypass cache
