...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cache-aware fetch. Default pakai cache 30s (lihat EtingDB).
    // Set { force: true } cuma kalau user mau data fresh secara eksplisit.
    function fetchLeaderboard(opts) {
        if (!leaderboardList) return;
        // Sumber data: Supabase langsung (lihat assets/js/supabase-client.js)
        if (!window.EtingDB || !window.EtingDB.getLeaderboard) {
            leaderboardList.innerHTML =
                '<div class="leaderboard-empty">Klien database belum siap.</div>';
            return;
        }
        leaderboardList.innerHTML =
            '<div class="leaderboard-empty">Memuat leaderboard…</div>';
        const p = window.EtingDB.getLeaderboard(50, opts || {})
            .then((data) => renderLeaderboard(data))
            .catch(() => {
                leaderboardList.innerHTML =
                    '<div class="leaderboard-empty">Tidak dapat memuat leaderboard.</div>';
            });
        // Loader halaman tunggu data ini selesai sebelum fade-out.
    ...
    // Filter (placeholder — user ganti filter = mau lihat data fresh
    // sesuai kriterianya, bypass cache).
