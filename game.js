...
(function () {
    ...
    const SFX = (function () {
        ...
        const aJump = load('sound/jump.mp3', 0.5);
        const aBonus = load('sound/bonus.mp3', 0.7);
        const aNabrak = load('sound/nabrak.mp3', 0.8);
        const aStart = load('sound/start.mp3', 0.6);
        const aWinning = load('sound/winning.mp3', 0.7);
    ...
    const assets = {
        ...
        coin: null,
        alas: null,
        sprite: null,     // sprite sheet burung enggang (sudah chroma-keyed)
        pipeReady: false,
    ...
    (function loadAssets() {
        ...
        alasImg.src = 'img/alas.png';

        // ===== Sprite burung enggang: load + chroma-key dengan FLOOD-FILL =====
        // PENTING: hanya hapus putih yang terhubung ke tepi gambar (background luar).
        // Putih di TENGAH (mata, highlight) yang dikurung garis hitam TIDAK dihapus.
        const spriteImg = new Image();
        spriteImg.onload = () => {
            try {
                const off = document.createElement('canvas');
                off.width = spriteImg.width;
                off.height = spriteImg.height;
                ...
                // Predicate "putih": min(R,G,B) >= threshold
                const WHITE_THRESH = 235;
                const isWhite = (idx) => {
                    const r = px[idx], g = px[idx + 1], b = px[idx + 2];
                    return Math.min(r, g, b) >= WHITE_THRESH;
                };

                // Flood-fill BFS dari semua pixel tepi (top/bottom row, left/right col)
                // Tandai mana yang "background luar" → set alpha 0.
                // Pakai Uint8Array sebagai visited mask (hemat memori).
                const visited = new Uint8Array(w * h);
                // Stack-based iterative (hindari rekursi: image bisa jutaan pixel)
                const stack = [];

                // Seed: semua pixel tepi yang putih
                ...
                while (stack.length) {
                    ...
                    if (x > 0) {
                        const np = p - 1;
                        if (!visited[np] && isWhite(np * 4)) { visited[np] = 1; stack.push(np); }
                    }
                    if (x < w - 1) {
                        const np = p + 1;
                        if (!visited[np] && isWhite(np * 4)) { visited[np] = 1; stack.push(np); }
                ...
                // Anti-alias edge: untuk pixel yang BELUM transparent tapi cukup terang
                // dan bertetangga dengan pixel transparent, turunkan alpha-nya sedikit.
                // Ini menghaluskan tepi sehingga tidak ada "halo" putih kasar.
                for (let y = 1; y < h - 1; y++) {
                    for (let x = 1; x < w - 1; x++) {
                        const p = y * w + x;
                        if (visited[p]) continue; // sudah transparan
                        const idx = p * 4;
                        const minRGB = Math.min(px[idx], px[idx + 1], px[idx + 2]);
                        if (minRGB < 200) continue; // bukan hampir-putih, skip
                        // Cek ada tetangga transparan?
                        const nT = visited[p - 1] || visited[p + 1] || visited[p - w] || visited[p + w];
                        if (nT) {
                            // Lerp alpha berdasar kecerahan: 200→255, 240→64
                            const t = Math.min(1, (minRGB - 200) / 40);
                            px[idx + 3] = Math.round(px[idx + 3] * (1 - t * 0.75));
                        }
                    }
                }
                ...
                console.warn('sprite chroma-key gagal:', err);
                assets.sprite = spriteImg;
                assets.spriteReady = true;
                bumpProgress('Sprite burung (mode fallback)');
            }
        };
        spriteImg.onerror = () => {
    ...
    const PUZZLE_IMAGES = ['img/tenun1.png', 'img/tenun2.png', 'img/tenun3.png', 'img/tenun4.png'];
    ...
    function drawPlayer() {
        ...
        if (assets.spriteReady && assets.sprite) {
            // Pakai sprite enggang (sudah chroma-keyed)
    ...
    function promptName() {
        ...
        if (inputPlayerSchool) {
            inputPlayerSchool.value = playerSchool || '';
        }
    }
    ...
    function submitScoreToLeaderboard() {
        ...
        if (score <= 0) return;
        if (score === lastSubmittedScore) return; // hindari dobel kirim skor yg sama
    ...
    if (btnToggleLeaderboard && gameLeaderboardPanel) {
        btnToggleLeaderboard.addEventListener('click', () => {
            ...
            // Saat dibuka, refresh data biar selalu fresh
            if (wasCollapsed) {
    ...
    // Muat leaderboard awal (panel di samping game)
    refreshLeaderboard();
